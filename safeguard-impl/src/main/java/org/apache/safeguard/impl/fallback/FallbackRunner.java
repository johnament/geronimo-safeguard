...
public class FallbackRunner {
    private final Class<FallbackHandler> handlerClass;
    private final String method;

    public FallbackRunner(Class<FallbackHandler> handlerClass, String method) {
    ...
    public Object executeFallback(InvocationContext invocationContext) {
        ...
        if(method != null) {
            try {
                Method method = invocationContext.getTarget().getClass().getMethod(this.method, ExecutionContext.class);
                return method.invoke(invocationContext.getTarget(), executionContext);
            } catch (Exception e) {
                throw new IllegalArgumentException(e);
            }
        }
        else {
            boolean cdiManaged = true;
            ...
            Object result = fallbackHandler.handle(executionContext);
            if(cdiManaged) {
                cdi.destroy(fallbackHandler);
            }
            return result;
        }
    }
